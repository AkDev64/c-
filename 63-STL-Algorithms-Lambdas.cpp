...
/*
  ...
  *   `std::transform`: Aplica uma função (ou lambda) a cada elemento de um
      ...
      Sintaxe: `std::transform(inicio_origem, fim_origem, inicio_destino,
  operacao);`

  *   `std::remove_if`: Remove elementos de um intervalo que satisfazem uma
                       determinada condição (definida por uma função ou lambda).
      IMPORTANTE: `remove_if` não redimensiona o contêiner. Ele move os
  elementos que NÃO devem ser removidos para o início do intervalo e retorna um
  iterador para o "novo final lógico". Você precisa usar o método `erase` do
  contêiner para realmente remover os elementos e redimensioná-lo. Sintaxe:
  `auto novo_fim = std::remove_if(inicio, fim, condicao);`
  ...
  *   **Sintaxe Básica:** `[capturas](parâmetros) -> tipo_retorno {
  corpo_da_função }`
      - `[capturas]`: Permite que a lambda "capture" variáveis do escopo onde
  ela foi definida.
          - `[]`: Não captura nada.
...
int main() {
  ...
  auto novo_fim_contar =
      std::remove_if(numeros_para_contar.begin(), numeros_para_contar.end(),
