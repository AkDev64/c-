...
/*
  ...
  1.  **Reduzir a verbosidade e melhorar a legibilidade:**
      Especialmente com tipos complexos da STL (como iteradores, tipos de
  retorno de lambdas, etc.). O código fica mais limpo e fácil de ler. Exemplo:
  `auto it = mapa_complexo.begin();`
  ...
  1.  **Quando a clareza é prejudicada:**
      ...
      Exemplo: `auto y = obterValor();` (Qual o tipo de 'y' sem ver
  'obterValor()'?)
...
int main() {
  ...
  std::cout << "inteiro: " << inteiro << " (tipo: " << typeid(inteiro).name()
            << "\\n";
  std::cout << "flutuante: " << flutuante
            << " (tipo: " << typeid(flutuante).name() << "\\n";
  ...
  auto it_vec =
      nomes.begin(); // Tipo deduzido: std::vector<std::string>::iterator
  std::cout << "\\nPrimeiro nome no vetor (via auto iterator): " << *it_vec
            << "\\n";

  std::map<int, std::string> mapa_idades = {{1, "Um"}, {2, "Dois"}};
  // Sem auto: std::map<int, std::string>::iterator it_map =
  // mapa_idades.begin();
  auto it_map =
      mapa_idades
          .begin(); // Tipo deduzido: std::map<int, std::string>::iterator
  std::cout << "Primeiro par no mapa (via auto iterator): " << it_map->first
            << ", " << it_map->second << "\\n";
  ...
  std::cout << "\\nvalor_int: " << valor_int
            << " (tipo: " << typeid(valor_int).name() << "\\n";
  std::cout << "valor_double_explicito: " << valor_double_explicito
            << " (tipo: " << typeid(valor_double_explicito).name() << "\\n";
  std::cout << "valor_double_auto: " << valor_double_auto
            << " (tipo: " << typeid(valor_double_auto).name() << "\\n";
